...
#fetch meterstand
METERSTAND=`curl -s --connect-timeout 2 --max-time 5 -H "Accept: application/json" "http://$DOMOTICZ_IP:$DOMOTICZ_PORT/json.htm?type=command&param=getdevices&rid=$IDX" | /bin/grep '"Counter" :' | awk {'print $3'} | /usr/bin/cut -d '"' -f 2`
if [ $METERSTAND ] ; then
#   echo $METERSTAND
