...
#        echo "$HOSTNAME : Domoticz at minute $MINUTE still OK!"
   ...
   if [ $STATUS2] ; then
      ...
      if [ $STATUS3 ] ; then
         ...
         lsof -i tcp:${DOMOTICZ_PORT} | awk 'NR!=1 {print $2}' | xargs kill -9   # Added 19-09-2018 clear used ports 8080
         sudo service domoticz.sh start
         curl -s -X POST $TELEGRAM_URL -d chat_id=$TELEGRAM_ID -d text="$(echo -e "$HOSTNAME: Domoticz Restarted!")" > /dev/null 2>&1
         logger -t Domoticz check -- Domoticz was offline. Restarted Domoticz....
